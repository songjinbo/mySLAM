...
//	pcl::visualization::CloudViewer viewer("viewer");
	...
	for(int currIndex = startIndex+1;currIndex<endIndex;currIndex++)
	{
		...
		CHECK_RESULT result = checkKeyFrames(keyframes.back(),currFrame,globalOptimizer,false,camera);
		switch(result)
		{
			case NOT_MATCHED:
			...
			case KEYFRAME:
				...
				{
					checkNearbyLoops(keyframes,currFrame,globalOptimizer,nearby_loops,camera);
					checkRandomLoops(keyframes,currFrame,globalOptimizer,random_loops,camera);
				}
...
void rgb2Gray(cv::Mat& src,cv::Mat& grey)
{
	grey.create(src.rows,src.cols,CV_8UC1);
	for(int y = 0;y<src.rows;y++)
	{
		for(int x = 0;x<src.cols;x++)
		{
			...
			int tmp = 0.2989*R+0.587*G+0.114*B;
			grey.at<unsigned char>(y,x) = tmp;
		}
	}
}
...
{
	//读取left和right文件
	std::string leftPath = path+"left/"+"000005_"+itos(index)+".png";
	std::string rightPath = path+"right/"+"000005_"+itos(index)+".png";
	...
	cv::Mat src_left = cv::imread(leftPath.c_str(), -1);
//	std::cout<<"the depth is : "<<src_left.depth()<<std::endl;
//	std::cout<<"the channels is : "<<src_left.channels()<<std::endl;
	cv::Mat src_right = cv::imread(rightPath.c_str(), -1);

	frame.depthColor = src_left;
	rgb2Gray(src_left,frame.left);
	rgb2Gray(src_right,frame.right);
	frame.frameID = index;
	return 1;
}
		...
		bool is_loops,CameraParam &camera)
{
	...
	MOTION result = motionEstimation(f1,f2,camera,good_match_threshold,min_good_match,min_inliers);	
	if(result.inliers < 0)
		return NOT_MATCHED;
