...
//	detectorAndDescriptor->detectAndCompute(frame.left,cv::noArray(),frame.kp,frame.desp);
	...
	/*
	cv::Mat imgShow;
	cv::drawKeypoints(frame.left,frame.kp,imgShow,cv::Scalar::all(-1),cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
...
*/
    ...
    {
        cv::Point2f p = frame1.kp[matches[i].queryIdx].pt;
        ushort d = frame1.depth.ptr<float>( int(p.y) )[ int(p.x) ];
		if(matches[i].distance == 0 || d==0) 
			continue;
...
//	std::cout<<"solving pnp"<<std::endl;
    ...
    cv::solvePnPRansac( pts_obj, pts_img, cameraMatrix, cv::Mat(), rvec, tvec,false,100,2.0,0.9,inliers);
    result.rvec = rvec;
    result.tvec = tvec;
