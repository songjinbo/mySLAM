...
//	std::cout<<sgbm->getNumDisparities()<<std::endl;
	...
	double max;//disp中元素的最大值	
	double *maxp = &max;
	cv::minMaxIdx(disp,NULL,maxp);
	cv::Mat disp64;
	disp.convertTo(disp64,CV_64F);
	disp64 = disp64*255/max;
	cv::Mat disp8u;
	disp64.convertTo(disp8u,CV_8U);

	cv::imshow("disparity",disp8u);
	cv::imwrite("disparity.png",disp8u);
	cv::waitKey(0);
